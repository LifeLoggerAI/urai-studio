...
assert.ok(narrow.includes('.studio-rail-v2 .rail-group-v2 {'), 'narrow Studio rail group override must exist')
assert.ok(narrow.includes('flex-wrap: wrap;'), 'narrow Studio rail links must wrap instead of clipping')
assert.ok(narrow.includes('overflow-x: visible;'), 'narrow Studio rail must not hide off-screen links')
assert.ok(narrow.includes('.studio-rail-v2 .rail-group-v2 p {'), 'narrow Studio rail group labels must retain their own row')
assert.ok(narrow.includes('flex-basis: 100%;'), 'narrow Studio rail group labels must span the row')
